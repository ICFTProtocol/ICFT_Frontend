...
export const metadata: Metadata = { title: "Get started", description: "Prepare a test-only wallet and explore ICFT collateralized borrowing on Ethereum Sepolia." };
...
export default function StartPage() {
  return <main className={styles.page}>
    ...
    <section className={`${styles.choose} shell`}><div className={styles.chooseHead}><span className="kicker">BORROWING FLOW</span><h2>Start with collateral.</h2><p>The live Sepolia flow uses protocol-owned ICFT credit inventory. It is designed for testing and interface validation only.</p></div><div className={styles.flowGrid}>{flows.map(([title, first, second, third, href]) => <article key={title}><span>01 / CREDIT</span><h3>{title}</h3><ol><li>{first}</li><li>{second}</li><li>{third}</li></ol><Link href={href}>Open borrowing flow ↗</Link></article>)}</div></section>
