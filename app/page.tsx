...
export default function Home() {
  ...
  return <main>
    ...
    {live.pool.paused && <section className="notice shell"><Glyph>!</Glyph> The Sepolia LendingPool is paused during an oracle security review. Deposits, borrowing and withdrawals are unavailable; use the dApp only to inspect positions or repay ICFT.</section>}
    ...
    <section className="protocol shell" id="protocol"><div className="protocolHeading"><span className="kicker">02 / THE SYSTEM</span><h2>One reserve.<br />Clear incentives.</h2></div><div className="flow"><Flow number="01" title="Deposit collateral" body="ETH and wBTC form the active collateral basket on this Sepolia baseline." /><Flow number="02" title="Borrow ICFT" body="Debt is USD-denominated and interest accrues through a global index." /><Flow number="03" title="Repay ICFT" body="Repaid principal returns to protocol credit inventory; interest follows the protocol reserve policy." /><Flow number="04" title="Keep the system whole" body="Liquidations protect the protocol reserve; insurance absorbs bad debt under the configured rules." /></div></section>
    ...
    <section className="faq shell" id="faq"><div><span className="kicker">03 / FIELD NOTES</span><h2>Questions, answered.</h2><a className="textLink" href="/faq">Browse all FAQ ↗</a></div><div className="faqList"><Faq q="Is this mainnet?" a="No. This interface points to Ethereum Sepolia. It is an engineering baseline for testing only." /><Faq q="Which collateral is active?" a="The active Sepolia collateral set is native ETH and wBTC. wstETH is disabled pending oracle remediation." /><Faq q="How is borrowing interest calculated?" a="Debt is denominated in USD and accrues through the LendingPool global borrow index. The rate model responds to pool utilization, so the rate is not fixed when a loan is opened." /><Faq q="Can I withdraw collateral at any time?" a="Only when the LendingPool is operational and the withdrawal keeps your position below the configured maximum LTV. Repay debt or add collateral first if the requested withdrawal would make the position unsafe." /><Faq q="Can I buy or sell ICFT here?" a="No official exchange route is configured in this testnet interface. Market data and verified buy or sell links will only be shown after an official trading venue is published." /><Faq q="What should I do before testing?" a="Use a Sepolia-only wallet. Never enter a seed phrase and never send mainnet assets to testnet contracts." /></div></section>
