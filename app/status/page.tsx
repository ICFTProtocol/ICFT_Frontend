...
const modules = [
  ...
  ["Liquidity vault", "liquidityVault"],
] as const;

export default function StatusPage() {
  const live = useProtocolData();
  const block = useBlockNumber({ watch: true, query: { refetchInterval: 15_000 } });
  const state = live.isError || block.isError ? "RPC UNAVAILABLE" : live.isLoading || block.isLoading ? "SYNCING" : "LIVE READ";
  ...
  return <main className={styles.page}>
    ...
    <section className={`${styles.metrics} shell`}><Metric label="Available liquidity" value={`${amount(live.pool.availableLiquidity, 0)} ICFT`} /><Metric label="Pool utilization" value={`${Number(live.pool.utilizationBps) / 100}%`} /><Metric label="LP assets" value={`${amount(live.pool.lpAssets, 0)} ICFT`} /><Metric label="Current borrow APR" value={`${Number(live.risk.borrowRateBps) / 100}%`} /></section>
    <section className={`${styles.prices} shell`}><div className={styles.sectionHead}><span className="kicker">ORACLE SNAPSHOT</span><h2>Collateral prices.</h2><p>USD values normalized by the PriceOracle. The page refreshes approximately every 15 seconds.</p></div><div className={styles.priceGrid}><Price symbol="ETH" label="Native Ether" value={money(live.prices.eth)} /><Price symbol="wBTC" label="Wrapped Bitcoin" value={money(live.prices.wbtc)} /><Price symbol="wstETH" label="Wrapped staked Ether" value={money(live.prices.wsteth)} /><Price symbol="ICFT" label="Protocol credit asset" value={money(live.prices.icft)} /></div></section>
