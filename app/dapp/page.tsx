...
export default function DappHome() {
  ...
  return <>
    ...
    <section className={styles.welcome} id="workspace"><div><span className={styles.eyebrow}>ICFT / CREDIT WORKSPACE</span><h2>Build a position<br /><em>with intent.</em></h2><p>Use approved collateral to access ICFT and manage your risk in one place. Credit inventory is protocol-owned, not publicly supplied.</p></div><div className={styles.welcomeActions}><button className={styles.primary} onClick={() => openAction("Deposit")}>Deposit collateral <b>+</b></button><a className={styles.ghost} href="/dapp/markets">View market status <b>↗</b></a></div></section>
...
function FlowFacts({ action, asset, approval, minimumBorrowUsd }: { action: Action; asset: string; approval: boolean; minimumBorrowUsd: bigint }) { return <div className={flow.facts}><span>{action === "Borrow" ? "Minimum borrow" : "Wallet balance"}</span><b>{action === "Borrow" ? usd(minimumBorrowUsd) : "Connect wallet to load"}</b><span>{action === "Withdraw" ? "Safety check" : "Approval"}</span><b>{action === "Withdraw" ? "LTV preview before confirmation" : approval ? `${asset} approval required` : "Not required"}</b></div>; }
