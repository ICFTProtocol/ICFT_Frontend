...
function FlowFacts({ action, asset, approval }: { action: Action; asset: string; approval: boolean }) { return <div className={flow.facts}><span>{action === "Borrow" ? "Minimum borrow" : action === "Redeem" ? "Available LP shares" : "Wallet balance"}</span><b>{action === "Borrow" ? "Protocol value pending" : "Connect wallet to load"}</b><span>{action === "Withdraw" ? "Safety check" : "Approval"}</span><b>{action === "Withdraw" ? "LTV preview before confirmation" : approval ? `${asset} approval required` : "Not required"}</b></div>; }
