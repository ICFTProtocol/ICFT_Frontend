...
export function WorkspaceView({ view }: { view: string }) {
  ...
  if (view === "liquidity") return <Page title="Liquidity desk" sub="Live LP vault state and your receipt shares">
    <section className={ui.overviewGrid}><Stat label="Pool assets" value={`${amount(data.pool.lpAssets)} ICFT`} detail="LendingPool LP assets" /><Stat label="Utilization" value={bps(data.pool.utilizationBps)} detail={`${amount(data.pool.availableLiquidity)} ICFT available`} /><Stat label="Your LP shares" value={amount(data.balances.lp, 4)} detail={connected ? "icftLP balance" : "Connect wallet to load"} /></section>
    <section className={ui.split}><article className={ui.card}><div className={ui.cardHead}><div><span>LP VAULT</span><h3>Supply or redeem.</h3></div><span className={ui.pill}>LIVE</span></div><p className={ui.copy}>Supply uses ICFT approval followed by vault supply. Redeem burns your icftLP shares and returns the available proportional ICFT value.</p><div className={ui.actionCards}><Action title="Supply ICFT" text="Mint real icftLP shares." href={action("Supply")} icon="↗" /><Action title="Redeem icftLP" text="Withdraw available ICFT." href={action("Redeem")} icon="↙" /></div></article><article className={`${ui.card} ${ui.allocation}`}><span>INTEREST ALLOCATION</span><div><b>85%</b><p>to LP liquidity</p></div><div><b>15%</b><p>to insurance reserve</p></div><small>Current reserve setting is configured in LendingPool. Confirm governance policy before production.</small></article></section>
  </Page>;

  if (view === "activity") return <Page title="Activity ledger" sub="On-chain events emitted for the connected wallet">
    <section className={ui.card}><div className={ui.feedToolbar}><div><button className={ui.selected}>Your events</button></div><span>SEPOLIA BLOCK {block.data?.toString() ?? "…"}</span></div>{!connected && <div className={ui.emptyState}><i>⌁</i><div><b>Connect a wallet to read its events</b><p>Activity is loaded directly from LendingPool and LP vault logs on Sepolia.</p></div></div>}{connected && activity.loading && <div className={ui.emptyState}><i>…</i><div><b>Reading protocol events</b><p>Loading deposit, borrow, repay, supply and redeem logs from Sepolia.</p></div></div>}{connected && !activity.loading && activity.items.length === 0 && <div className={ui.emptyState}><i>⌁</i><div><b>No ICFT events for this wallet</b><p>Your confirmed protocol operations will appear here directly from emitted contract events.</p></div></div>}{activity.items.map((item) => <Feed key={item.id} title={item.title} value={item.amount} detail={`${item.detail} · block ${item.block.toString()}`} ok={item.ok} />)}</section>
