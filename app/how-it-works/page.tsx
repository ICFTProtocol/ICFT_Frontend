...
export default function HowItWorksPage() {
  return <main className={styles.page}>
    ...
    <section className={`${styles.split} shell`}><article><span className="kicker">FOR BORROWERS</span><h2>Your position has a health limit.</h2><p>Borrowing is limited to 80% LTV in the current testnet configuration. The liquidation threshold is 90%. Asset prices and accrued interest can change your LTV even while you are away, so leaving a buffer matters.</p><Link href="/faq" className={styles.textLink}>Learn about liquidation risk ↗</Link></article><article><span className="kicker">PROTOCOL CREDIT RESERVE</span><h2>Credit inventory is protocol-owned.</h2><p>The initial Fund A reserve supplies borrowable ICFT. The public interface does not offer a user LP product, yield promise or user-controlled lending inventory.</p><Link href="/dapp/liquidity" className={styles.textLink}>View market status ↗</Link></article></section>

    <section className={`${styles.risk} shell`}><div><span>RISK IS PART OF THE DESIGN</span><h2>What protects<br />the reserve?</h2></div><div className={styles.riskList}><p><b>Price feeds</b>Collateral is valued through the configured PriceOracle, with freshness checks for oracle data.</p><p><b>Collateral limits</b>The RiskEngine enforces maximum LTV and the liquidation threshold before borrowing or withdrawals.</p><p><b>Liquidations</b>When a position is unsafe, a permitted liquidator can repay debt and seize collateral under the protocol rules.</p><p><b>Insurance first</b>Realized bad debt is first directed to the insurance reserve before it affects protocol credit inventory.</p></div></section>
